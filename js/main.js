(() => {
  ...
  function init() {
    ...
    const personOrder = D.relationships.map(r => r.personId);
    ...
    personList.addEventListener('click', e => { const b = e.target.closest('[data-person]'); if (b) select(b.dataset.person, { fromUser: true }); });
    workList.addEventListener('click', e => { const b = e.target.closest('[data-work]'); if (b) select(relByWork[b.dataset.work].personId, { fromUser: true }); });

    /* ---------- Build: legend ---------- */
    const legendList = $('#legend-list');
    Object.entries(D.statuses).forEach(([key, s]) => {
      const li = document.createElement('li');
      li.className = 'status'; li.dataset.status = key;
    ...
    D.sources.forEach(s => {
      ...
      a.href = s.url; a.target = '_blank'; a.rel = 'noopener noreferrer'; a.textContent = s.title;
      const hidden = document.createElement('span'); hidden.className = 'sr-only'; hidden.textContent = ' (mở trong tab mới)';
      a.appendChild(hidden);
      const pub = document.createElement('span'); pub.textContent = s.publisher;
      li.append(a, pub); sourceList.appendChild(li);
    });

    /* ---------- Build: careers ---------- */
    Object.entries(D.careers).forEach(([id, c]) => {
      const btn = document.createElement('button');
      btn.type = 'button'; btn.className = 'career-tab'; btn.id = `tab-${id}`;
      btn.setAttribute('role', 'tab'); btn.setAttribute('aria-controls', 'career-panel');
      btn.dataset.career = id; btn.textContent = c.label;
      careerTabs.appendChild(btn);
    });
    D.person.forEach(p => {
      const b = document.createElement('button');
      b.type = 'button'; b.className = 'chip-btn'; b.dataset.chip = p.id;
      b.setAttribute('aria-pressed', 'false'); b.textContent = p.label;
      careerChips.appendChild(b);
    });
    careerChips.addEventListener('click', e => { const b = e.target.closest('[data-chip]'); if (b) select(b.dataset.chip, { fromUser: true, scene: false }); });
    ...
    function renderCareer() {
      ...
      D.work.forEach(w => {
        ...
        wrap.className = 'attr'; wrap.dataset.person = rel.personId;
        ...
        c[w.id].forEach(text => { const li = document.createElement('li'); li.textContent = text; ul.appendChild(li); });
        const q = document.createElement('p'); q.className = 'attr-q'; q.textContent = rel.question;
        dd.append(ul, q);
        wrap.append(dt, dd);
    ...
    careerTabs.addEventListener('click', e => { const t = e.target.closest('[data-career]'); if (!t) return; state.careerId = t.dataset.career; renderCareer(); });
    careerTabs.addEventListener('keydown', e => {
      ...
      state.careerId = tabs[n].dataset.career; renderCareer();
      tabs[n].scrollIntoView({ block: 'nearest', inline: 'nearest', behavior: reduceMotion.matches ? 'auto' : 'smooth' });
    });

    /* ---------- Select a pair (from either side, or from the scroll scrubber) ---------- */
    function select(personId, { announce = true, scene = true } = {}) {
      ...
      $('#scrub-now').textContent = String(personOrder.indexOf(personId) + 1).padStart(2, '0');
      ...
      rel.sources.forEach(id => {
        ...
        a.href = s.url; a.target = '_blank'; a.rel = 'noopener noreferrer'; a.textContent = s.title;
        const hidden = document.createElement('span'); hidden.className = 'sr-only'; hidden.textContent = ' (mở trong tab mới)';
        a.appendChild(hidden); src.appendChild(a);
      });

      if (scene && !reduceMotion.matches) {
        reading.classList.remove('is-updated'); void reading.offsetWidth; reading.classList.add('is-updated');
      }

      paintCareerLens();
      drawPath(scene);
      if (announce) live.textContent = `Đang xem: ${person.label} và ${work.label}. ${status.label}. ${rel.question}`;
    }
    ...
    function drawPath(animate) {
      ...
      fieldGrad.setAttribute('x1', x1.toFixed(1)); fieldGrad.setAttribute('x2', x2.toFixed(1));
      fieldMid.setAttribute('cx', mx.toFixed(1)); fieldMid.setAttribute('cy', ((y1 + y2) / 2).toFixed(1));
      if (animate && !reduceMotion.matches) { fieldPath.classList.remove('is-drawing'); void fieldPath.getBoundingClientRect(); fieldPath.classList.add('is-drawing'); }
    }
    ...
    /* Exposed for js/motion.js, which drives selection from scroll position. */
    window.__modelSelect = select;
    window.__personOrder = personOrder;
    ...
    /* ---------- Header current-section + fold accordion ---------- */
    if ('IntersectionObserver' in window) {
      const links = $$('a[href^="#"]', $('.nav'));
      const spy = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          if (!entry.isIntersecting) return;
          links.forEach(l => { if (l.getAttribute('href') === `#${entry.target.id}`) l.setAttribute('aria-current', 'true'); else l.removeAttribute('aria-current'); });
        });
      }, { rootMargin: '-45% 0px -50% 0px' });
      $$('main > section[id]').forEach(s => spy.observe(s));
    }

    $$('.fold-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const item = btn.closest('.fold-item');
        const open = item.classList.contains('is-open');
        $$('.fold-item').forEach(i => { i.classList.remove('is-open'); $('.fold-btn', i).setAttribute('aria-expanded', 'false'); });
    ...
    /* ---------- Initial render ---------- */
    renderCareer();
    select(state.personId, { announce: false, scene: false });
  }
})();
